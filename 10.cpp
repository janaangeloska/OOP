...
private:
    ...
    int n;
public:
    Oglasnik(char *imeNaOglasnik="", Oglas *oglasi= nullptr, int n=0){
    ...
    Oglasnik(const Oglasnik &ogl){
        strcpy(this->imeNaOglasnik,ogl.imeNaOglasnik);
        for(int i=0;i<n;i++){
    ...
    Oglasnik &operator +=(Oglas &oglas){
        if(oglas.getCena()<0){
            throw NegativnaVrednost((char *)"Oglasot ima nevalidna vrednost za cenata i nema da bide evidentiran!");
        }
