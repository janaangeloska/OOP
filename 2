...
//NOTICE: vo ovaa zadaca default vrednosti se postaveni "nullptr" zaradi courses (ne prifakja stringovi)
...
protected:
    ...
    bool veteran;
public:
    Vozac(char *ime=nullptr, int vozrast=18, int brTrki=0, bool veteran=false) {
        strcpy(this->ime,ime);
    ...
    friend ostream &operator<<(ostream &out, Vozac &vozac) {
        out << vozac.ime <<endl;
        out << vozac.vozrast <<endl;
        out << vozac.brTrki <<endl;
        if(vozac.veteran) {
            out << "VETERAN" << endl;
        }
        return out;
    ...
    bool operator==(Vozac &a){
        return zarabotuvacka() == a.zarabotuvacka();
    }
...
private:
    double cenaNaAvtomobilot;
public:
    Avtomobilist(char *ime=nullptr, int vozrast=0, int brTrki=0, bool veteran=false, double cenaNaAvtomobilot=0 ): Vozac(ime,vozrast,brTrki,veteran){
        this->cenaNaAvtomobilot=cenaNaAvtomobilot;
    }
    ~Avtomobilist(){}
    double zarabotuvacka(){
        return cenaNaAvtomobilot/5;
    }
    double danok(){
        double danok;
        if(brTrki>10){
            danok=0.15*zarabotuvacka();
        }
        else{
            danok=0.1*zarabotuvacka();
        }
        return danok;
    }
};
...
private:
    int mokjnost;
public:
    Motociklist(char *ime=nullptr, int vozrast=0, int brTrki=0, bool veteran=false, int mokjnost=0 ): Vozac(ime,vozrast,brTrki,veteran) {
    ...
    double danok(){
        double danok;
        if(veteran){
            danok=0.25*zarabotuvacka();
        }
        else{
            danok=0.2*zarabotuvacka();
        }
        return danok;
    }
    };
int soIstaZarabotuvachka(Vozac *v[],int n,Vozac *a){
    int brojac=0;
    for(int i=0;i<n;i++){
        if(v[i]->zarabotuvacka()==a->zarabotuvacka()){
            brojac++;
        }
    }
    return brojac;
